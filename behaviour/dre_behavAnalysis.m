...
%    the RSA analysis.
% ~~~
% GX Castegnetti --- start ~ 07.06.18 --- last ~ 24.08.18
...
%% subjects and trials
% the script must be run twice: once for sub#003 with 160 trials, and once
% for everybody else with 120 trials
subs = [4:5 8 9 13:17 19:21 23 25:27 29:32 34:37 39];
ntrials = 120;

%% task order (1: FB; 2:BF)
...
pl_his_SS = false;
pl_den_SS = false;
hist_fire = [0.75 0.5 0];
hist_boat = [0.35 0.4 0.8];
...
ratDiffAll_F = [];
ratDiffAll_B = [];
if pl_rda_SS, hRSA = figure('color',[1 1 1]); end %#ok<*UNRCH>
...
for s = 1:length(subs)
    ...
    data_F = csvread([dirNew,'/data/behaviour/SF',num2str(subs(s),'%03d'),'/SF',num2str(subs(s),'%03d'),'_B1_DRE.csv']);
    data_B = csvread([dirNew,'/data/behaviour/SF',num2str(subs(s),'%03d'),'/SF',num2str(subs(s),'%03d'),'_B2_DRE.csv']);
    ...
    %% day 1 - plot value and confidence
    
    if pl_his_SS
        % plot rating for each item
        figure('color',[1 1 1]) 
        subplot(2,1,1), bar(rat_F(s,:),'facecolor',hist_fire),title(['sub#',num2str(subs(s))])
        set(gca,'XTick',1:ntrials,'fontsize',11,'XtickLabel',objsName),xtickangle(45)
        subplot(2,1,2), bar(rat_B(s,:),'facecolor',hist_boat)
        set(gca,'XTick',1:ntrials,'fontsize',11,'XtickLabel',objsName),xtickangle(45)
        
        % plot rating and confidence histograms
        figure('color',[1 1 1])  
        subplot(2,2,1),histogram(rat_F(s,:),20,'facecolor',hist_fire)
        title(['Sub#',num2str(subs(s)),'; Task: F - Rating'],'fontsize',18), set(gca,'fontsize',14)
        subplot(2,2,3),histogram(rat_B(s,:),20,'facecolor',hist_boat)
        title(['Sub#',num2str(subs(s)),'; Task: B - Rating'],'fontsize',18), set(gca,'fontsize',14)
        subplot(2,2,2),histogram(con_F(s,:),20,'facecolor',hist_fire)
        title(['Sub#',num2str(subs(s)),'; Task: F - Confid.'],'fontsize',18), set(gca,'fontsize',14)
        subplot(2,2,4),histogram(con_B(s,:),20,'facecolor',hist_boat)
        title(['Sub#',num2str(subs(s)),'; Task: B - Confid.'],'fontsize',18), set(gca,'fontsize',14)
    end
    ...
    figure('color',[1 1 1])
    subplot(1,2,1)
    plot(xspan,sigm_F_rht,'linewidth',5,'color',hist_fire), hold on
    plot(xspan,sigm_F_opp,'linewidth',5,'color',hist_boat),title(['sub#',num2str(subs(s),'%3d')])
    plot(ratingDiff_F,(Cho_F(:,3)+1)/2,'linestyle','none','marker','.','markersize',25,'color','k')
    set(gca,'fontsize',24),ylabel('P(Right)'),xlabel('Value difference')
    
    subplot(1,2,2)
    plot(xspan,sigm_B_rht,'linewidth',5,'color',hist_boat),hold on
    plot(xspan,sigm_B_opp,'linewidth',5,'color',hist_fire)
    plot(ratingDiff_B,(Cho_B(:,3)+1)/2,'linestyle','none','marker','.','markersize',25,'color','k')
    set(gca,'fontsize',23),ylabel('P(Right)'),xlabel('Value difference')
