...
%% correlation between values and confidences
...
for s = 1:length(subs)
    ...
    set(gca,'XTick',1:5,'YTick',1:5,'fontsize',11,'XtickLabel',{'vF','vB','cF','cB','P'},'YtickLabel',{'vF','vB','cF','cB','P'})
end, clear s

% plot average
figure('color',[1 1 1])
imagesc(squeeze(mean(cMat,3))), title('average','fontsize',16), caxis([-1 1])
set(gca,'XTick',1:5,'YTick',1:5,'fontsize',16,'XtickLabel',{'vF','vB','cF','cB','P'},'YtickLabel',{'vF','vB','cF','cB','P'})

% compute statistics
[~,pCorr_valFvsB,~,stats] = ttest(squeeze(cMat(1,2,:))); %#ok<ASGLU>
[~,pCorr_conFvsB,~,stats] = ttest(squeeze(cMat(2,3,:)));

%% stats for choice

% left/right
[~,p_LR,~,stats_LR] = ttest(choices_L,choices_R);
...
% choice vs val
slopesCongr = [slopes.FonF] + [slopes.BonB];
slopesIncon = [slopes.FonB] + [slopes.BonF];
