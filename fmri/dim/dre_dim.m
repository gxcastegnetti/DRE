...
for s = 1:length(subNames)
    ...
    sessType1 = bData(s).sessType{1};
    idx_s1 = bData(s).imagination(1).(sessType1).objIdx;
    if strcmp(sessType1,'boat')
        idx_s1 = idx_s1 + 120;
    end
    
    sessType2 = bData(s).sessType{2};
    idx_s2 = bData(s).imagination(2).(sessType2).objIdx;
    if strcmp(sessType2,'boat')
        idx_s2 = idx_s2 + 120;
    end
    
    sessType3 = bData(s).sessType{3};
    idx_s3 = bData(s).imagination(3).(sessType3).objIdx;
    if strcmp(sessType3,'boat')
        idx_s3 = idx_s3 + 120;
    end
    
    sessType4 = bData(s).sessType{4};
    idx_s4 = bData(s).imagination(4).(sessType4).objIdx;
    if strcmp(sessType4,'boat')
        idx_s4 = idx_s4 + 120;
    end
    
    % for every subject, this should be nVox x nCond x nRuns
    respPatt_2goals{s}(:,:,1) = responsePatterns.(subNames{s})(:,idx_s1);
    respPatt_2goals{s}(:,:,2) = responsePatterns.(subNames{s})(:,idx_s2);
    respPatt_2goals{s}(:,:,3) = responsePatterns.(subNames{s})(:,idx_s3);
    respPatt_2goals{s}(:,:,4) = responsePatterns.(subNames{s})(:,idx_s4);
end
...
%% subjects
[bestn,r_outer, r_alter, test_tfce] = functional_dimensionality(respPatt_2goals, binaryMask, 'sphere',9);