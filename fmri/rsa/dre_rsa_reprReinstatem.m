...
for r = 1:length(roiNames)
    for s = 1:length(subs)
        ...
        for trlCho = 1:96
            ...
            % identify chosen and unchosen
            if ObjChosen_allSessions(trlCho) == -1
                respPattIma_chosenItem = respPattIma_leftSide;
                respPattIma_unchosItem = respPattIma_righSide;
...
%             
%             % stack which object was chosen
%             val_allSessions = [bData(subs(s)).imagination(1).val;
...
%             
%             % identify more and less valuable object
...
%             end
            ...
            corr_choiceVSchosen(trlCho) = corr(respPattCho_trl,respPattIma_chosenItem);
            corr_choiceVSunchos(trlCho) = corr(respPattCho_trl,respPattIma_unchosItem);
        end
...
%% plot for paper
means = [0.00096,0.0016,0.00033;0.0042,0.0059,0.0024];
sems = [0.0021,0.0030,0.0032;0.0020,0.0019,0.0036];

roiNamesTrue = {'Left HPC','Right HPC'};
myColors = [0.5,0.5,0.5; 0.3,0.3,0.3; 0.7,0.7,0.7];
figure('color',[1 1 1])
...
legend({'All subs','Best performing subs','Worst performing subs'},'location','northwest'), legend boxoff
