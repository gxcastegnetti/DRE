...
%% run searchlight for value, confidence, familiarity, price, object ID
...
for s = 1:length(subs)
    ...
    model(1).color = [0 1 0];
    model(2).name = 'confidence';
    model(2).RDM = RDMs{s}.con;
    model(2).color = [0 1 0];
    model(3).name = 'familiarity';
    ...
    save([dir.out,fs,analysisName,fs,'sl_SF',num2str(subs(s),'%03d')],'rs')
