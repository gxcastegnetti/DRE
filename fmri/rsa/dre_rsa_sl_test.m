...
if true
    for s = 1:length(subs)
        ...
        for m = 1:length(modelNames)
            ...
            % take smoothed data
            smoothMetadataStruct_sS = spm_vol(swrMapFile);
            smoothData = spm_read_vols(smoothMetadataStruct_sS);
            
            % apply mask
            smoothData(mask_sS == 0) = nan;
            
            % write again
            spm_write_vol(smoothMetadataStruct_sS, smoothData);
