...
%
%                s
%                        subject number (added because different subjects have 
%                        different beta correspondences).
%
...
Subject = userOptions.subjectNames{s}; % <- this thing before only took the first subject (see next two lines)
% ~~~
...
% ~~~
% The reason is that here each object is a condition, and sessions are made
% of different objects for different subjects. Hence, every subject has a
% different beta correspondence.
% ~~~
...
% find number of betas and conditions
...
for betaNumber = 1:nBetas % For each beta file...
    ...
    [thisBetaName, ~, thisConditionName] = extractSingleBetaInfo(thisBetaDescrip);
    thisSessionNumber = 1;
    
    % Check if it's one of the conditions
    thisBetaIsACondition = false;
    for condition = 1:nConditions
        ...
        if thisBetaIsACondition, break; end
    ...
    if thisBetaIsACondition
        ...
        betas(thisSessionNumber, conditionNumber).identifier = [thisBetaName '.nii'];  %#ok<*AGROW>
    end
end
