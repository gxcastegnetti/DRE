function spmMat_name = dre_L1_iVF_cV(dir,analysisName,subs,timing,bData)
...
for s = 1:length(subs)
    ...
    for r = 1:n_sess
        ...
        job1LM{1}.spm.stats.fmri_spec.sess(r).cond(1).pmod(2).name = 'imagination_famil';
        job1LM{1}.spm.stats.fmri_spec.sess(r).cond(1).pmod(2).param = bData(subs(s)).imagination(r).(sessType).value;
