function spmMat_name = dre_L1_ikF_ckU(dir,analysisName,subs,timing,bData)
%% function dre_L1_ikVCF_ckV(dirSub,sub,runType)
% ~~~
% First level analysis with conditions:
%   * familiarity  H/L
%   * chosen - unchosen value  H/L
% ~~~
% GX Castegnetti --- 2018
...
for s = 1:length(subs)
    ...
    for r = 1:n_sess
        ...
        %% imagination familiarity

        % low
        job1LM{1}.spm.stats.fmri_spec.sess(r).cond(1).name = ['ima. low conf. - ',sessType];
        job1LM{1}.spm.stats.fmri_spec.sess(r).cond(1).onset = bData(subs(s)).imagination(r).k_fam.low.onset + timing.iOns;
        ...
        job1LM{1}.spm.stats.fmri_spec.sess(r).cond(2).onset = bData(subs(s)).imagination(r).k_fam.high.onset + timing.iOns;
        job1LM{1}.spm.stats.fmri_spec.sess(r).cond(2).duration = timing.iDur;
        job1LM{1}.spm.stats.fmri_spec.sess(r).cond(2).tmod = 0;

        %% value chosen - value unchosen

        % low
        job1LM{1}.spm.stats.fmri_spec.sess(r).cond(3).name = ['low chosen val. - ',sessType];
        job1LM{1}.spm.stats.fmri_spec.sess(r).cond(3).onset = bData(subs(s)).choice(r).k_chMunc.low.onset + timing.cOns;
        ...
        job1LM{1}.spm.stats.fmri_spec.sess(r).cond(4).onset = bData(subs(s)).choice(r).k_chMunc.high.onset + timing.cOns;
