function spmMat_name = dre_L1_ikV_ckV(dir,analysisName,subs,timing,bData)
...
for s = 1:length(subs)
    ...
    for r = 1:n_sess
        ...
        % low
        job1LM{1}.spm.stats.fmri_spec.sess(r).cond(3).name = ['choice low dV - ',sessType];
        job1LM{1}.spm.stats.fmri_spec.sess(r).cond(3).onset = bData(subs(s)).choice(r).k_valDiff.low.onset + timing.cOns;
        job1LM{1}.spm.stats.fmri_spec.sess(r).cond(3).duration = timing.cDur;
        job1LM{1}.spm.stats.fmri_spec.sess(r).cond(3).tmod = 0;
        
        % high
        job1LM{1}.spm.stats.fmri_spec.sess(r).cond(4).name = ['choice high dV - ',sessType];
        job1LM{1}.spm.stats.fmri_spec.sess(r).cond(4).onset = bData(subs(s)).choice(r).k_valDiff.high.onset + timing.cOns;
